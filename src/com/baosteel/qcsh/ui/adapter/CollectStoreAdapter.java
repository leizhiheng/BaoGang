 ...
 * 
 */
public class CollectStoreAdapter extends BaseSwipeAdapter {

	/**上下文**/
	private Context context;
	
	public List<StoreCollect> dataList;
	...
	public CollectStoreAdapter(Context context, boolean swipeEnable) {
		this.context = context;
		this.dataList = new ArrayList<StoreCollect>();
	 ...
	 * @param dataList
	 */
	public void appendData(List<StoreCollect> tempData) {
	 ...
	 * @param dataList
	 */
	public void refreshData(List<StoreCollect> tempData) {
	...
	class ViewHolder {
		 ...
		 * @param store
		 */
		void show(StoreCollect store, int position){
			
			ImageManager.Load(store.getBackground_img_app(), productImg);
			nameTv.setText(store.getMerchant_name());
			priceTv.setText("");
		}
	...
	public void fillValues(int position, View convertView) {
		...
		//显示数据
		viewHolder.show((StoreCollect)getItem(position), position);
	}
	...
	protected void onDeleteClick(int position) {
		...
		// 显示数据
		StoreCollect product= dataList.get(position);
		deleteCoupon(product.getMerchant_id(), position);
		
	}
	
	//***********swipe end***********
	
	private void deleteCoupon(final String merchantId, final int position) {

		deleteAppStoreCollect( merchantId, position);
...
//		RequstClient.appMyCouponDelete(handler, id);
	 ...
	 * @Time 2015-9-29
	 */
	public void deleteAppStoreCollect(String merchantId, final int position) {
		RequestClient.deleteAppStoreCollect(context, merchantId,BaoGangData.getInstance().getUserId(),  new RequestCallback<JSONObject>() {
