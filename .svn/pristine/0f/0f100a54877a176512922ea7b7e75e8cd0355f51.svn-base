	...
	private void initView() {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				...
				position -= headCount;
				if ("1".equals(adapter.getData().get(position).getStatus())) {
					Intent intent = new Intent(mContext, StoreMainActivity.class);
					//因为PullToRefreshListView的第一个Item是HeaderView，所以这个get()的时候参数要减1
					intent.putExtra(StoreMainActivity.MERCHANT_ID, adapter.getData().get(position).getMerchant_id());
					mContext.startActivity(intent);
				}else {
					startActivity(new Intent(mContext, StoreOrProductEmptyActivity.class).putExtra(StoreOrProductEmptyActivity.TIP_TYPE, StoreOrProductEmptyActivity.TIP_TYPE_STORE));
				}
			}
		});
